...
export function Navbar() {
  ...
  const navLinks = [
    { href: '/', label: 'Cartelera', icon: '🎬' },
    { href: '/validar', label: 'Validar tiquete', icon: '✅' },
    ...(profile?.rol === 'admin' ? [{ href: '/admin', label: 'Panel Admin', icon: '⚙️' }] : []),
  ]
    ...
    <>
      <nav style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
          ...
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #2563eb, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, boxShadow: '0 4px 12px rgba(37,99,235,0.3)' }}>🎬</div>
            ...
            {user ? (
              <div style={{ position: 'relative' }}>
                <button onClick={() => setDropOpen(!dropOpen)} style={{
                  ...
                  border: '1.5px solid #e2e8f0', borderRadius: 40, padding: '5px 14px 5px 6px',
                  ...
                  <>
                    ...
                    <div className="animate-slide-down" style={{
                      ...
                      {[
                        ...
                        <Link key={item.href} href={item.href} onClick={() => setDropOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', color: '#374151', textDecoration: 'none', fontSize: 14, fontWeight: 500, transition: 'background 0.15s' }}
                      ...
                      <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '11px 16px', color: '#ef4444', background: 'none', border: 'none', textAlign: 'left', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', transition: 'background 0.15s' }}
      ...
      {mobileOpen && (
        <div className="mobile-nav animate-slide-down" style={{
          ...
          {/* Nav links */}
          <div style={{ padding: '1rem 1rem', flex: 1 }}>
            ...
            {navLinks.map(link => (
              <Link key={link.href} href={link.href} style={{
                ...
                padding: '13px 12px', borderRadius: 12, fontSize: 15, fontWeight: 600,
                color: isActive(link.href) ? '#2563eb' : '#374151',
                background: isActive(link.href) ? '#eff6ff' : 'transparent',
                textDecoration: 'none', marginBottom: 4, transition: 'all 0.15s',
              }}>
                <span style={{ width: 36, height: 36, borderRadius: 10, background: isActive(link.href) ? '#dbeafe' : '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
                  {link.icon}
                </span>
                {link.label}
                {isActive(link.href) && (
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ marginLeft: 'auto' }}><polyline points="20 6 9 17 4 12"/></svg>
                )}
              </Link>
            ))}
            ...
            {user && (
              <>
                ...
                <Link href="/tiquetes" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 12px', borderRadius: 12, fontSize: 15, fontWeight: 600, color: '#374151', background: 'transparent', textDecoration: 'none', marginBottom: 4 }}>
                  <span style={{ width: 36, height: 36, borderRadius: 10, background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>🎟️</span>
                  Mis Tiquetes
                </Link>
              </>
            )}
